...
| `--study`           | No       | SE / CSV_ALL     | Study filter: single ID, comma-separated IDs, or `ALL`. In `se` mode, if omitted or set to `ALL`, all `.rda` files in `--icb_data_dir` are processed. In `csv_all`, it filters which `*_expr.csv` / `*_clin.csv` pairs run. | `--study ICB_small_Liu`<br>`--study ICB_small_Liu,ICB_small_Hugo`<br>`--study ALL` |
| `--expr_csv`        | **Yes**  | CSV only         | Expression CSV basename under `--icb_data_dir` (expects `<expr_csv>.csv`; genes × samples; rownames = genes).                                                                                                               | `--expr_csv ICB_small_Liu_expr`                                                    |
| `--clin_csv`        | **Yes**  | CSV only         | Clinical CSV basename under `--icb_data_dir` (expects `<clin_csv>.csv`; sample-level metadata). Must include at least `cancer_type` and `treatment`, plus required OS/PFS/response fields for the selected analyses.        | `--clin_csv ICB_small_Liu_clin`                                                    |
| `--study_id`        | **Yes**  | CSV only         | Cohort label used for output naming (also used as the study ID in results).                                                                                                                                                 | `--study_id ICB_small_Liu`                                                         |
| `--icb_data_dir`    | No       | All              | Input directory containing `.rda` files and/or paired `*_expr.csv` / `*_clin.csv` files.                                                                                                                                    | `--icb_data_dir ./ICB_data`                                                        |
| `--sig_data_dir`    | No       | All              | Directory containing signature `.rda` files. Each file must load an object named `sig`.                                                                                                                                     | `--sig_data_dir ./SIG_data`                                                        |
| `--sig_summary_dir` | No       | All              | Directory containing `signature_information.csv` (maps signature name → scoring method).                                                                                                                                    | `--sig_summary_dir ./sig_summery_info`                                             |
